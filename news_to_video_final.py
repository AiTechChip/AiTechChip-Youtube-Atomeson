...
--------------------------------------------------------------------------------
  ...
  [CHANGED] SCRIPT_PROMPT_TEMPLATE rewritten for an English viral tech
  ...
  [ADD] Daily queue + smart ranking: scan_fresh_candidates() + load_queue()
        feed a single score_candidate_stories() NVIDIA LLM call; top 3 get
  ...
  [KEPT] The v3 fixes: mark-processed-after-primary-delivery ordering (now
         ...
         delivery channels), fuzzy dedup, NVIDIA LLM call budget + pacing per
  ...
  [ADD]  _ensure_shorts_hashtag(): guarantees '#Shorts' is present in the
         hashtag list (and therefore the description) even if the NVIDIA LLM forgets
  ...
  [CHANGED] LLM provider switched from Google Gemini to NVIDIA NIM
            (build.nvidia.com) - uses the standard `openai` SDK pointed at
            NVIDIA's OpenAI-compatible endpoint (_build_nvidia_client()).
            GEMINI_API_KEY -> NVIDIA_API_KEY, GEMINI_MODEL -> NVIDIA_MODEL
            (default "meta/llama-3.1-70b-instruct" - NVIDIA's model catalog
            changes over time, so verify this ID still exists at
            build.nvidia.com and swap it via the NVIDIA_MODEL env var/config
            if not). JSON-mode is NOT assumed guaranteed across NVIDIA-hosted
            models (support varies by model/NIM release), so this relies on
            the existing prompt instruction ("respond only with JSON") plus
            the already-present regex-based _extract_json_block() fallback
            rather than a provider-enforced JSON mode - unchanged behavior
            either way since that fallback already existed for Gemini too.
  [FIX]  NEWS_RSS_URL switched from Google's general "top headlines" feed to
         the Technology topic feed (.../rss/headlines/section/topic/
         TECHNOLOGY?...). The general feed's top ~40 entries are mostly
         non-tech world news, so whitelist matching against TechCrunch/The
         Verge/etc frequently found zero candidates in a batch (observed in
         production: "Found 0 fresh whitelisted candidate(s)"). The topic
         feed is tech-focused by construction, making whitelist hits far
         more reliable.
...
!pip install -q feedparser openai edge-tts moviepy pillow numpy requests \
...
# ------------------------------------------------------------------------------
NEWS_RSS_FEEDS = [
    # General tech/AI/company news
    "https://news.google.com/rss/headlines/section/topic/TECHNOLOGY?hl=en-US&gl=US&ceid=US:en",
    # Cybersecurity / hacking
    "https://news.google.com/rss/search?q=cybersecurity%20OR%20hacking%20OR%20data%20breach&hl=en-US&gl=US&ceid=US:en",
    # Tech tips / how-to
    "https://news.google.com/rss/search?q=tech%20tips%20OR%20how-to%20smartphone%20OR%20windows%20tips&hl=en-US&gl=US&ceid=US:en",
    # Robotics
    "https://news.google.com/rss/search?q=robotics%20OR%20robot&hl=en-US&gl=US&ceid=US:en",
]
...
MAX_CANDIDATES_TO_SCORE     = 15       # cap on candidates sent to the single ranking NVIDIA LLM call
...
SOURCE_WHITELIST: Dict[str, List[str]] = {
    # General tech / big company / gadget news
...
def scan_fresh_candidates(
    ...
    """Scans EVERY feed in NEWS_RSS_FEEDS (general tech, cybersecurity/hacking,
    tech-tips, robotics) for fresh, whitelisted headlines not already in the
    processed log or in the current queue/exclude list. Results across feeds
    are merged and deduped, then capped to max_count."""
...
AI & Technology video channel covering AI tools, productivity, gadgets, big tech company news, cybersecurity/hacking, tech tips & how-tos, and robotics - the whole "internet/tech world" beat.
...
SCRIPT_PROMPT_TEMPLATE = """You are an expert viral tech scriptwriter for a global/US English
YouTube Shorts / Instagram Reels / TikTok channel covering the whole "internet/tech world"
beat for a US audience: AI tools, productivity, gadgets, big tech company news,
cybersecurity/hacking, tech tips & how-tos, and robotics.
